...
public class ListPlageDataFDBActivity extends AppCompatActivity implements SERVER_ADDRESS {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
              ...
              //  Intent i = new Intent(ProduitEAVActivity.this, AddMovieActivity.class);
                Intent i = new Intent(ListPlageDataFDBActivity.this, PlageDataFDB.class);
...
private void loadPlageForCreateTxFraisDeblocage(){
    ...
    try {
        ...
        if (success == 1) {
            ...
            for (int i = 0; i < CreateProduitCredit.plageDataListFDB.size(); i++) {
                ...
                String movieName =  CreateProduitCredit.plageDataListFDB.get(i).getPdValDe()+
                        " - "+ CreateProduitCredit.plageDataListFDB.get(i).getPdValA()+
                        " ( "+ CreateProduitCredit.plageDataListFDB.get(i).getPdValTaux()+" )";
                HashMap<String, String> map = new HashMap<String, String>();
                map.put(KEY_FDB_ID, movieId.toString());
                map.put(KEY_FDB_LIBELLE, movieName);
                movieList.add(map);
            }
        }
    } catch (Exception e) {
...
private void loadPlageForUpdateFDB(){
    ...
    try {
        ...
        if (success == 1) {
            ...
            for (int i = 0; i < movies.length(); i++) {
                 JSONObject movie = movies.getJSONObject(i);
                Integer PdNumero = movie.getInt(KEY_FDB_ID);
                String PdTypeData = movie.getString(KEY_FDB_LIBELLE);
                HashMap<String, String> map = new HashMap<String, String>();
                map.put(KEY_FDB_ID, PdNumero.toString());
                map.put(KEY_FDB_LIBELLE, PdTypeData);
                movieList.add(map);
            }
        }
    } catch (Exception e) {
...
private class FetchListTxFraisEtudeDossierAsyncTask extends AsyncTask<String, String, String> {
    ...
    protected void onPreExecute() {
        ...
        pDialog = new ProgressDialog(ListPlageDataFDBActivity.this);
...
if (IS_TO_CREATE_OR_TO_UPDATE){
    loadPlageForCreateTxFraisDeblocage();
}else loadPlageForUpdateFDB();
    ...
    private void populateMovieListPlageTxFraisEtudeDossier() {
        ListAdapter adapter = new SimpleAdapter(
                ListPlageDataFDBActivity.this, movieList,
                R.layout.list_item, new String[]{KEY_FDB_ID,
                KEY_FDB_LIBELLE},
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                if (CheckNetworkStatus.isNetworkAvailable(getApplicationContext())) {
                    ...
                    Intent intent = new Intent(getApplicationContext(),
                            PlageDataFDB.class);
                    if (IS_TO_CREATE_OR_TO_UPDATE){
                        PlageDataFDB.what_to_do=2;  //Pour la mise à jour d'un produit en cours d'enregistrement
                    }else{
                        PlageDataFDB.what_to_do=3; //Pour la mise à jour d'un produit déjà enregistré
                    }

                    intent.putExtra(KEY_FDB_ID, movieId);
                    startActivityForResult(intent, 20);

                } else {
                    Toast.makeText(ListPlageDataFDBActivity.this,
